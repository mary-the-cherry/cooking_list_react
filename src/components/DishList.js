...
export default function DishList(props) {
    ...
    function deleteIngridientDish(id) {
        const reducedIngridients = ingridientsDish.filter(ingridient => ingridient.id !== id);
        ...
        <li className="dish list-group-item">
            <div className="dish-name row">
                ...
                <span className="btn-group col-2">
                    ...
                    >Save <span className="visually-hidden">{props.dishname}</span></button>
                    <button
                        type="button"
                        className="btn btn-secondary btn-sm"
                        onClick={() => props.deleteDish(props.id)}
                    >Delete <span className="visually-hidden">{props.dishname}</span></button>
                </span>
            </div>
        ...
        <li className="dish list-group-item">
            <div className="dish-name row ">
                ...
                <span className="btn-group col-2">
                    ...
                    >Edit <span className="visually-hidden">{props.dishname}</span></button>
                    <button
                        type="button"
                        className="btn btn-secondary btn-sm"
                        onClick={() => props.deleteDish(props.id)}
