...
function App(props) {
    ...
    const dishList = dishes.filter(FILTER_MAP[filter]).map(dish => <DishList
        dishname={dish.name}
    ...
    function addDish(name) {
        ...
        const newDish = { id: "dish-" + nanoid(), name: name, completed: false, ingridients: ingridients };
    ...
    function deleteIngridient(id) {
        const reducedIngridients = ingridients.filter(ingridient => ingridient.id !== id);
        ...
        <div className="background">
            <div className="app-complete container">
                <header className="app-header">
                    ...
                    <h5 className="introduction">Track your cooking ideas on his side and always have an overview what you need to buy for it.</h5>
                        ...
                        {showShoppingList && 
                            <div className="pop-up">
                                <div className="pop-up-inner">
                                    <h3>Shopping List</h3>
                                    <ul>
                                        {shoppingList.map(ingridient => <li key={nanoid()}>{ingridient}</li>)}
                                    </ul>
                                    <button
